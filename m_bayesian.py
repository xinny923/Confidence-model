"""
Bayesian-weighted modify trials (E = F * (1 - M)) using 4-term model.
Softmax-based M (as in msoft), outputs to output_M/ with fig3-6 and robustness.
"""
...
class Config:
    ...
    OUTPUT_DIR = Path("output_bayesian")
    DATA_DIR = Path("Data")
    CODE_DIR = Path("Code")
    PLOT_DPI = 300
    RANDOM_SEED = 42
    ...
    for t in range(Config.NUM_TRIALS):
        ...
        if not math.isnan(feedback2):
            if action_label == "modify":
                F = feedback2 / 5  # normalize to +/-1
                weight = (1 - M)
                E_eff = F * weight
                if E_eff > 0:
                    e_matrix[t, 0] = abs(E_eff)  # e1 (accept +)
                elif E_eff < 0:
                    e_matrix[t, 2] = abs(E_eff)  # e3 (accept -)
            else:
...
def main():
    ...
    trial_df.to_csv(Config.OUTPUT_DIR / "trial_data_bayes.csv", index=False)
    ...
    scores.to_csv(Config.OUTPUT_DIR / "participant_summary_bayes.csv", index=False)
    ...
    model_params.to_csv(Config.OUTPUT_DIR / "table1_model_params_bayes.csv", index=False)
    ...
    overall = run_overall_logit(logit_df); overall.to_csv(Config.OUTPUT_DIR / "table2_logit_coeffs_bayes.csv", index=False)
    group_logit = run_category_logit(logit_df, score_groups); group_logit.to_csv(Config.OUTPUT_DIR / "table3_category_logit_bayes.csv", index=False)

    # Robustness
    robust = run_robustness(matrices)
    robust_stats = summarize_robustness(robust, model_params)
    robust_stats.to_csv(Config.OUTPUT_DIR / "robustness_statistics_bayes.csv", index=False)
    final_params = []
    for name in ["aiconf", "selfconf"]:
        final_params.append({
            "model": name,
    ...
    pd.DataFrame(final_params).to_csv(Config.OUTPUT_DIR / "final_robust_params_bayes.csv", index=False)

    print("\n=== Bayesian M-weighted model (4-term) ===")
