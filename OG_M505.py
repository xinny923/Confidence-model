"""
OG_M505: OG 4-term model with modify split 50/50.

Update equations (Chong et al. style):

  C(n+1) = C(n) + alpha_e [E(n) - C(n)] + alpha_a [A(n) - C(n)] + alpha_b [B(n) - C(n)]
  where C(n), E(n), A(n), B(n), alpha_e, alpha_a, alpha_b in [0, 1].

  A(n) = gamma * C(n-1) + (1 - gamma) * A(n-1),
  where gamma in [0, 1], and A(0) = C(0).

  E(n) = omega1 * e1(n) + omega2 * e2(n) + omega3 * e3(n) + omega4 * e4(n) + gamma_res * Align(n),
  where e1(n), e2(n), e3(n), e4(n) are 0 or 1,
  omega1, omega2, omega3, omega4 in [0, 1],
  and sum_{i=1}^4 e_i(n) = 1 (modify trials split 0.5/0.5 into accept/reject of same sign).

Notes:
  - This script uses the same update equations as benchmark_rigor (no physical/momentum terms).
  - Evidence uses 4 experiences; modify trials are split 0.5/0.5 into accept/reject of same sign.
  - Mean-data R^2 with factor 99/92, MSE = ssres/60 (2 cond x 30 trials).
  - Robustness: 100 iterations, 80 participants each.
Outputs to output_og_m505/.
"""
...
class Config:
    ...
    INITIAL = {
        "aiconf": [0.2672, 0.3405, 0.0524, 0.8439, 0.2115, 0.0, 0.5217, 0.3897, 0.02],
        "selfconf": [0.2844, 0.4706, 0.0, 0.5736, 0.8284, 0.2384, 0.2863, 0.1147, 0.02],
    }

    DATA_DIR = Path("Data")
    CODE_DIR = Path("Code")
    OUTPUT_DIR = Path("output_og_m505")
    PLOT_DPI = 300
    RANDOM_SEED = 42
...
def parse_participant(path: Path, pid: int, condition: int, base_id: int) -> ParticipantData:
    ...
    for t in range(Config.NUM_TRIALS):
        ...
        if action == 0:
            idx = sign + 1  # accept: e1 or e3
            if 0 <= idx < 4:
                row_e[idx] = 1.0
        elif action == 1:
            idx = sign + 2  # reject: e2 or e4
            if 0 <= idx < 4:
                row_e[idx] = 1.0
        else:
            # modify split 50/50 into accept/reject of same sign
            if sign == -1:
                row_e[0] = 0.5  # accept+
                row_e[1] = 0.5  # reject+
            elif sign == 1:
...
def build_align_tensor(trial_df: pd.DataFrame, participants: List[ParticipantData]) -> np.ndarray:
    pid_to_idx = {p.pid: i for i, p in enumerate(participants)}
    align = np.zeros((len(participants), Config.NUM_TRIALS), dtype=float)
    for _, row in trial_df.iterrows():
        pid = int(row["pid"])
        idx = pid_to_idx[pid]
        t = int(row["trial"]) - 1
        orig = str(row.get("original_move", ""))
        ai = str(row.get("ai_suggestion", ""))
        if orig == ai and orig not in {"", "None", "nan"}:
            align[idx, t] = 1.0
    return align


def build_matrices(parts: List[ParticipantData], trial_df: pd.DataFrame) -> Dict[str, np.ndarray]:
    return {
        ...
        "align_tensor": build_align_tensor(trial_df, parts),
...
def simulate_confidence(params: np.ndarray, observed: np.ndarray, e_tensor: np.ndarray, align_tensor: np.ndarray) -> np.ndarray:
    alpha_e, alpha_a, alpha_b, omega1, omega2, omega3, omega4, gamma, gamma_res = params
...
def fit_model(observed: np.ndarray, e_tensor: np.ndarray, align_tensor: np.ndarray, initial: np.ndarray) -> Dict[str, Any]:
    bounds = (np.zeros_like(initial), np.ones_like(initial))
    result = least_squares(
        simulate_confidence,
        x0=initial,
        bounds=bounds,
        args=(observed, e_tensor, align_tensor),
        max_nfev=5000,
    )
    # mean-data R2
    # Use only valid (non-NaN) trials for mean-data R2
    pred_all = compute_predictions(result.x, observed, e_tensor, align_tensor)
    ...
    residuals = simulate_confidence(result.x, observed, e_tensor, align_tensor)
    return {"params": result.x, "mse": mse_mean, "adj_r2": adj_r2, "residuals": residuals}


def compute_predictions(params: np.ndarray, observed: np.ndarray, e_tensor: np.ndarray, align_tensor: np.ndarray) -> np.ndarray:
    alpha_e, alpha_a, alpha_b, omega1, omega2, omega3, omega4, gamma, gamma_res = params
    n_participants, series_len = observed.shape
    n_trials = series_len - 1
    c = observed[:, 0].copy()
...
def run_robustness(matrices: Dict[str, np.ndarray]) -> Dict[str, Any]:
    ...
    for _ in range(Config.ROBUSTNESS_N_ITER):
        ...
        for name, mat in [("aiconf", matrices["ai_conf_matrix"]), ("selfconf", matrices["self_conf_matrix"])]:
            ...
            fit_res = fit_model(obs, e_sub, align_sub, initial)
...
def plot_figure3(model_params: pd.DataFrame, matrices: Dict[str, np.ndarray], output_dir: Path):
    ...
    for col, cond in enumerate([1, 2]):
        ...
        ai_pred = compute_predictions(ai_params, matrices["ai_conf_matrix"][mask], e_sub, matrices["align_tensor"][mask])[:, 1:]
        ...
        self_pred = compute_predictions(self_params, matrices["self_conf_matrix"][mask], e_sub, matrices["align_tensor"][mask])[:, 1:]
...
def main():
    ...
    matrices = build_matrices(participants, trial_df)
    ...
    for name, conf_matrix in [("aiconf", matrices["ai_conf_matrix"]), ("selfconf", matrices["self_conf_matrix"])]:
        initial = np.array(Config.INITIAL[name], dtype=float)
        fit_res = fit_model(conf_matrix, matrices["e_tensor"], matrices["align_tensor"], initial)
