function [mse,rsq] = modelPlot_self(x)
...
for cond=1:2
    ...
    semSelfC = std(selfC_data,1)./sqrt(size(selfC_data,1));

    % ------------------------Confidence plot--------------------------------
    % Plot data
    figure;
    hold on;
    p1 = errorbar(0:30,meanSelfC,semSelfC,'k.','LineWidth',1.5,'MarkerSize',25);
    ...
    selfC_data_new = selfC_data(:,2:end);
    p = polyfit(x1,selfC_data_new(:)',8);
    est = polyval(p,1:30);
    ssres8 = ssres8 + sum((est-meanSelfC(2:end)).^2);
    sstot8 = sstot8 + sum((meanSelfC(2:end)-mean(meanSelfC(2:end))).^2);
    p6 = plot(est);
    %----------------------------------------------------------------------
...
% 
%     save(strcat('fitselfC',num2str(cond),'.mat'),'C');
    %------------------------------------------------------------------------
...
%         'Performance change','Unexpected poor AI suggestions','Unexpected good AI suggestions','FontSize',15);
    ...
    ssres = ssres + sum((fitMean(2:end)-meanSelfC(2:end)).^2);
    sstot = sstot + sum((meanSelfC(2:end)-mean(meanSelfC(2:end))).^2);
%     if cond==1
%         mse1 = ssres/30;
%         rsq1 = 1-ssres/sstot;
%     else
%         mse2 = sum((fitMean(2:end)-meanSelfC(2:end)).^2)/30;
%         rsq2 = 1-sum((fitMean(2:end)-meanSelfC(2:end)).^2)/sum((meanSelfC(2:end)-mean(meanSelfC(2:end))).^2);
%     end

    res = [res fitMean(2:end)-meanSelfC(2:end)];
...
plot(fitted,res,'.');
hold on;
plot(linspace(0.4,0.55),zeros(100,1));
axis([0.4 0.55 -0.1 0.1])
xlabel('Fitted Data');
ylabel('Residual');
title('Residual Plot - Self-Confidence');
