"""
Extended analysis with softmax-based M weighting for modify trials (4-term model).
Outputs to output_M/, reproducing Table1/fig3-6 style and robustness.
"""
...
class ParticipantData:
    ...
    act_series: np.ndarray  # 0/1 accept, but action_label kept in trial records
    ...
    for t in range(Config.NUM_TRIALS):
        ...
        if not math.isnan(feedback2):
            if action_label == "modify":
                if feedback2 > 0:
                    e_matrix[t, 0] = 1 - M  # e1 (accept+)
                    e_matrix[t, 1] = M      # e2 (reject+)
                elif feedback2 < 0:
                    e_matrix[t, 2] = 1 - M  # e3 (accept-)
                    e_matrix[t, 3] = M      # e4 (reject-)
            else:
...
def fit_model(observed: np.ndarray, e_tensor: np.ndarray, initial: np.ndarray) -> Dict[str, Any]:
    ...
    adj_r2 = 1 - (99 / 92) * (ssres / sstot) if sstot > 0 else float("nan")  # k=7 -> n-k-1=92
...
def main():
    ...
    trial_df.to_csv(Config.OUTPUT_DIR / "trial_data_M.csv", index=False)
    ...
    scores.to_csv(Config.OUTPUT_DIR / "participant_summary_M.csv", index=False)
    ...
    model_params.to_csv(Config.OUTPUT_DIR / "table1_model_params_M.csv", index=False)
    ...
    overall = run_overall_logit(logit_df); overall.to_csv(Config.OUTPUT_DIR / "table2_logit_coeffs_M.csv", index=False)
    group_logit = run_category_logit(logit_df, score_groups); group_logit.to_csv(Config.OUTPUT_DIR / "table3_category_logit_M.csv", index=False)

    # Robustness
    robust = run_robustness(matrices)
    robust_stats = summarize_robustness(robust, model_params)
    robust_stats.to_csv(Config.OUTPUT_DIR / "robustness_statistics_M.csv", index=False)
    final_params = []
    for name in ["aiconf", "selfconf"]:
        final_params.append({
            "model": name,
    ...
    pd.DataFrame(final_params).to_csv(Config.OUTPUT_DIR / "final_robust_params_M.csv", index=False)

    print("\n=== Softmax M-weighted model (4-term) ===")
