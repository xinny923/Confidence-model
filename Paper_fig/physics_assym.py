"""
physics_assym: asymmetric force for AI vs Self with lagged memory.
...
class Config:
    ...
    DATA_DIR = Path("Data")
    OUTPUT_DIR = Path("Paper_fig") / "physics_assym_output"
    PLOT_DPI = 300
    RANDOM_SEED = 42
    MAX_NFEV = 2000
    DELTA_WEIGHT = 0.0
...
def simulate_residuals(params: np.ndarray, observed: np.ndarray,
                       e_tensor: np.ndarray, use_self_force: bool) -> np.ndarray:
...
def fit_model(obs: np.ndarray, e_tensor: np.ndarray,
    ...
    res = least_squares(
        ...
        x0=init,
        bounds=bounds,
        args=(obs, e_tensor, use_self_force),
        max_nfev=Config.MAX_NFEV,
    )
    pred_all = compute_predictions(res.x, obs, e_tensor, use_self_force)
    params = res.x
...
def run_robustness(matrices: Dict[str, np.ndarray]) -> Dict[str, Any]:
    ...
    for i in range(n_iter):
        ...
        for model_name, conf_matrix in [
            ...
            fit_res = fit_model(obs_sub, e_sub, init, (low, high), model_name == "selfconf", cond_sub)
...
def main() -> None:
    ...
    df_params.to_csv(Config.OUTPUT_DIR / "physics_assym_params.csv", index=False)

    print("\nPhysics_assym fit results:")
