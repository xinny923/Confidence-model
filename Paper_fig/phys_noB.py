"""
phys_noB: physics update without B term.
...
Force (Self and AI):
  F(n) = alpha_e*(E(n) - C(n)) + alpha_a*(A(n) - C(n))
...
class Config:
    ...
    INITIAL_GUESS = {
        "aiconf": [0.40, 0.30, 0.84, 0.21, 0.50, 0.00, 0.50, 0.50],
        "selfconf": [0.40, 0.30, 0.57, 0.83, 0.50, 0.00, 0.50, 0.50],
    }
    BOUNDS_LOW_DEFAULT = np.array([0, 0, 0, 0, 0, 0, 0, 0], dtype=float)
    BOUNDS_HIGH_DEFAULT = np.array([2, 2, 1, 1, 1, 1, 5, 1], dtype=float)
    BETA_FIXED = 0.0
    ...
    DATA_DIR = Path("Data")
    OUTPUT_DIR = Path("Paper_fig") / "phys_noB_output"
    PLOT_DPI = 300
    RANDOM_SEED = 42
    MAX_NFEV = 2000
    DELTA_WEIGHT = 0.0
...
def simulate_residuals(params: np.ndarray, observed: np.ndarray,
    ...
    alpha_e, alpha_a, o1, o2, o3, o4, beta, gamma = params
...
def main() -> None:
    ...
    df_params.to_csv(Config.OUTPUT_DIR / "phys_noB_params.csv", index=False)

    print("\nPhys_noB fit results:")
